...
function ht(h) {
  let t = h.reduce((s, c) => s + cv(c), 0);
  let a = h.filter(c => c.v === "A").length;
...
export default {
  ...
  async execute({ message, args, db, embed, prefix }) {
    ...
    const p = prefix;
    const inGame = sessions.has(uid) && ["hit","stand","double","h","s","d"].includes(sub);

    if (!inGame) {
      const bet = parseInt(sub);
      const u = await db.getUser(uid);
      if (isNaN(bet) || bet <= 0) {
        return message.channel.send({ embeds: [embed(COLORS.error).setDescription(`❌ e.g. \`${p}bj 500\``)] });
      }
      if (bet > u.bal) {
        return message.channel.send({ embeds: [embed(COLORS.error).setDescription("❌ Insufficient FC.")] });
      }
      if (bet > 500_000) {
        return message.channel.send({ embeds: [embed(COLORS.error).setDescription("❌ Max 500,000 FC.")] });
      }
      ...
      return message.channel.send({ embeds: [
        embed(COLORS.accent)
          .setTitle("🃏 Blackjack")
          .setDescription(
            `Your hand: **${fmt(player)}** (${ht(player)})\n` +
    ...
    const { deck, player, dealer } = sess;

    if (["hit","h","double","d"].includes(sub)) {
      if (["double","d"].includes(sub)) {
        const u = await db.getUser(uid);
        if (u.bal < sess.bet) {
          return message.channel.send({ embeds: [embed(COLORS.error).setDescription("❌ Not enough FC to double.")] });
        }
        sess.bet = sess.bet * 2;
        sess.doubled = true;
      }
    ...
    if (["stand","s"].includes(sub) || sess.doubled) {
      ...
      const dt = ht(dealer);
      const push = pt === dt;
      const won = pt > dt || dt > 21;

      if (push) {
        return message.channel.send({ embeds: [
          embed(COLORS.warn)
            .setTitle("🃏 PUSH")
            .setDescription(`You: **${fmt(player)}** (${pt}) | Dealer: **${fmt(dealer)}** (${dt})\nBet returned.`)
        ]});
      }
