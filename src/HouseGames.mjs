...
// ── Dice (over/under) ───────────────────────────────────────────────────
// Pick a target t in [2,98] and mode 'under'/'over'. Roll is a float in [0,100).
// Win: under → roll < t; over → roll > t. Win prob = t/100 (under) | (100-t)/100 (over).
// Payout mult = EDGE / winProb → RTP = EDGE (~3% edge). Max payout at t=2 (under) ≈ 48.5×.
export function dice(bet, target, mode) {
  const m = mode === "over" ? "over" : "under";
  let t = Math.floor(Number(target) || 0);
  t = Math.max(2, Math.min(98, t));
  const roll = +(rnd() * 100).toFixed(2); // 0.00..99.99
  const win = m === "under" ? roll < t : roll > t;
  const prob = m === "under" ? t / 100 : (100 - t) / 100;
  const mult = +(EDGE / prob).toFixed(4);
  return {
    mode: m,
    target: t,
    roll,
    win,
    chance: +(prob * 100).toFixed(2),
    mult,
...
export const CHICKEN = { lanes: 18, dMin: 0.1, dMax: 0.5, maxMult: 5000 };
...
export class HouseState {
  ...
  startChicken(uid, bet) {
    ...
    return {
      ...
      mult: 1,
      mults,
      deaths: deaths.map((d) => Math.round(d * 100)),
      nextMult: mults[0],
      nextDeath: Math.round(deaths[0] * 100),
    };
  }
  chickenStep(uid) {
    const g = this.chicken.get(uid);
    if (!g || g.over) return { error: "No active game" };
    if (rnd() < g.deaths[g.step]) {
      g.over = true;
      this.chicken.delete(uid);
      return { dead: true, step: g.step };
    }
    g.step++;
    const mult = g.mults[g.step - 1];
    if (g.step >= g.mults.length) {
      // crossed the whole road → forced cashout
      g.over = true;
      this.chicken.delete(uid);
      return {
        alive: true,
    ...
    return {
      ...
      mult,
      payout: Math.round(g.bet * mult),
      nextMult: g.mults[g.step],
      nextDeath: Math.round(g.deaths[g.step] * 100),
    };
  }
  chickenCashout(uid) {
    const g = this.chicken.get(uid);
    ...
    if (g.step < 1) {
      this.chicken.delete(uid);
      return { cancelled: true, refund: g.bet, bet: g.bet, step: 0 };
    }
    const mult = g.mults[g.step - 1];
    this.chicken.delete(uid);
  ...
  minesReveal(uid, idx) {
    ...
    if (!g || g.over) return { error: "No active game" };
    idx = idx | 0;
    if (idx < 0 || idx > 24 || g.revealed.includes(idx))
  ...
  minesCashout(uid) {
    ...
    if (!g.revealed.length) {
      this.mines.delete(uid);
      return { cancelled: true, refund: g.bet, bet: g.bet, bombs: [...g.grid] };
    }
    const mult = this.minesMult(g, g.revealed.length);
    const bombs = [...g.grid];
    this.mines.delete(uid);
  ...
  hiloOdds(rank) {
    ...
    return {
      higherMult: +(EDGE / pHi).toFixed(3),
      lowerMult: +(EDGE / pLo).toFixed(3),
    };
  }
  hiloGuess(uid, dir) {
    ...
    return Object.assign(
      {
        ...
        prev,
        next,
        mult: +g.mult.toFixed(3),
        payout: Math.round(g.bet * g.mult),
      },
      this.hiloOdds(next),
    );
  }
  hiloCashout(uid) {
    ...
    if (g.mult <= 1) {
      this.hilo.delete(uid);
      return { cancelled: true, refund: g.bet, bet: g.bet };
    }
...
export const HOUSE_GAMES = [
  {
    id: "plinko",
    name: "Plinko",
    tag: "Drop the ball, chase the multiplier",
    color: "#a855f7",
  },
  {
    id: "mines",
    name: "Mines",
    tag: "Dodge the bombs, cash out anytime",
    color: "#ef4444",
  },
  {
    id: "coinflip",
    name: "Coinflip",
    tag: "Heads or tails — 1.96×",
    color: "#eab308",
  },
  {
    id: "hilo",
    name: "HiLo",
    tag: "Higher or lower, build a streak",
    color: "#22c55e",
  },
  {
    id: "double",
  ...
  },
  {
    id: "chicken",
    name: "Chicken Road",
    tag: "Cross the road, dodge the cars, cash out",
    color: "#f97316",
  },
  {
    id: "dice",
    name: "Dice",
    tag: "Roll under or over — set your own odds",
    color: "#14b8a6",
  },
  {
    id: "crash",
